    ...
    <SidebarProvider>
      ...
      <SidebarInset className="min-w-0 bg-[#fbfaf7]">
        ...
        <main className="mx-auto w-full max-w-[1500px] p-4 md:p-8">
          ...
          {view === "usuarios" && supervisor && <UsersView data={data} actorEmail={data.actor.email} openNew={() => { setEditingUser(null); setModal("usuario"); }} openEdit={(user) => { setEditingUser(user); setModal("usuario"); }} reload={() => load()} />}
...
function UserStatusButton({ user, reload }: { user: User; reload: () => Promise<void> }) {
  const [saving, setSaving] = useState(false);
  const nextActive = !user.active;
  async function updateStatus() {
    setSaving(true);
    try {
      await requestJson(`/api/users/${encodeURIComponent(user.email)}`, {
        method: "PATCH",
        body: JSON.stringify({
          displayName: user.displayName,
          role: user.role,
          canRecordDeliveries: user.canRecordDeliveries,
      ...
      toast.error(error instanceof Error ? error.message : "Erro ao alterar o acesso.");
    } finally {
      setSaving(false);
    }
  }

  return <AlertDialog><AlertDialogTrigger asChild><Button type="button" size="sm" variant="outline">{user.active ? "Inativar" : "Reativar"}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{user.active ? "Inativar" : "Reativar"} {user.displayName}?</AlertDialogTitle><AlertDialogDescription>{user.active ? "O usuário será desconectado e não poderá entrar no sistema até ser reativado." : "O usuário poderá voltar a entrar no sistema com a senha já cadastrada."} A ação ficará registrada na auditoria.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction disabled={saving} onClick={() => void updateStatus()} className="bg-[#b63f2b] hover:bg-[#953321]">{saving ? "Salvando..." : user.active ? "Confirmar inativação" : "Confirmar reativação"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>;
}
...
function UsersView({ data, actorEmail, openNew, openEdit, reload }: { data: DashboardData; actorEmail: string; openNew: () => void; openEdit: (user: User) => void; reload: () => Promise<void> }) {
  return <Panel title="Usuários e permissões">{data.users.length ? <Table><TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>E-mail</TableHead><TableHead>Perfil</TableHead><TableHead>Entregas</TableHead><TableHead>Pagamentos</TableHead><TableHead>Acesso</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{data.users.map((user) => { const editingSelf = user.email === actorEmail; return <TableRow key={user.email}><TableCell className="font-semibold">{user.displayName}</TableCell><TableCell>{user.email}</TableCell><TableCell className="capitalize">{user.role}</TableCell><TableCell>{user.role === "supervisor" || user.canRecordDeliveries ? "Permitido" : "Consulta"}</TableCell><TableCell>{user.role === "supervisor" || user.canRecordPayments ? "Permitido" : "Consulta"}</TableCell><TableCell>{user.hasPassword ? "Configurado" : "Senha pendente"}</TableCell><TableCell>{user.active ? "Ativo" : "Inativo"}</TableCell><TableCell><div className="flex min-w-max justify-end gap-2"><Button type="button" size="sm" variant="outline" onClick={() => openEdit(user)}>Configurar</Button>{editingSelf ? <span className="self-center text-xs text-[#8d796b]">Conta atual</span> : <><UserStatusButton user={user} reload={reload} /><DeleteButton endpoint={`/api/users/${encodeURIComponent(user.email)}`} label="Excluir usuário" detail={`O acesso de ${user.displayName} será removido. Entregas, pagamentos e registros de auditoria já existentes serão preservados.`} reload={reload} /></>}</div></TableCell></TableRow>; })}</TableBody></Table> : <Empty title="Nenhum usuário" text="Cadastre quem poderá acessar o controle." action="Cadastrar usuário" onAction={openNew} />}</Panel>;
}
